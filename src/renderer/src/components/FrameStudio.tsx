...
function StudioLayer({ layer, selected, editing, onPointerDown, onResize, onSelect, onEdit, onTextChange, onFinishEdit }: { layer: FrameLayer; selected: boolean; editing: boolean; onPointerDown: (event: ReactPointerEvent) => void; onResize: (event: ReactPointerEvent) => void; onSelect: () => void; onEdit: () => void; onTextChange: (text: string) => void; onFinishEdit: () => void }): JSX.Element {
  ...
  if (layer.type === 'text') {
    ...
    return <button type="button" className={`studio-canvas-item studio-text ${selected ? 'selected' : ''}`} style={textStyle} onPointerDown={onPointerDown} onDoubleClick={onEdit}>{layer.text}{handle}</button>
  }
