...
export function SessionList({ language, sessions, templates, onBack, onCreate, onOpen, onDelete }: { language: Language; sessions: BoothSession[]; templates: TemplateManifest[]; onBack: () => void; onCreate: () => void; onOpen: (session: BoothSession) => void; onDelete: (session: BoothSession) => void }): JSX.Element {
  return <section className="sessions-page" aria-labelledby="sessions-title">
    <header className="sessions-heading"><div><p className="eyebrow">{tr(language, 'Thư viện cục bộ', 'Local library')}</p><h1 id="sessions-title">{tr(language, 'Các phiên chụp.', 'Your sessions.')}</h1><p>{tr(language, 'Mở lại bất kỳ phiên nào để sắp ảnh, chụp lại hoặc xuất bản in.', 'Open any session to rearrange photos, retake a slot, or prepare a print.')}</p></div><button className="primary-button" onClick={onCreate}>{tr(language, 'Tạo phiên mới', 'Create new session')}</button></header>
    ...
    <button className="back-link" onClick={onBack}>← {tr(language, 'Trang chính', 'Main page')}</button>
  </section>
}
